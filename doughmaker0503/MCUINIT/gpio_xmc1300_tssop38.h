...
/* Generated for XMC1300_TSSOP38 on: Wed Feb  7 12:48:22 2013*/

#include <XMC1100.h>  

//---------参照参考手册 寄存器部分  P1090
#define INPUT           0x00U   //直接输入
#define INPUT_PD        0x08U  //下拉
#define INPUT_PU        0x10U   //上拉
#define INPUT_PPS       0x18U
#define INPUT_INV       0x20U    //反向输入
#define INPUT_INV_PD    0x28U
#define INPUT_INV_PU    0x30U
#define INPUT_INV_PPS   0x38U
#define OUTPUT_PP_GP    0x80U   //通用输出    推挽
#define OUTPUT_PP_AF1   0x88U   //复用输出功能1
#define OUTPUT_PP_AF2   0x90U   //复用输出功能2
...
#define OUTPUT_OD_GP    0xC0U   // 漏极开路输出
...
__STATIC_INLINE void P1_2_set_mode(uint8_t mode){
    PORT1->IOCR0 &= ~0x00f80000UL;//先把对应的5位清空
    PORT1->IOCR0 |= mode << 16;//然后把函数参数对应入寄存器																		
}
